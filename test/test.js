...
tape( 'the function returns `true` if provided `+-0`', function test( t ) {
	t.strictEqual( isEvenf( +0.0 ), true, 'returns expected value' );
	t.strictEqual( isEvenf( -0.0 ), true, 'returns expected value' );
	t.end();
});

tape( 'WARNING: the function returns `true` if provided `+infinity`', function test( t ) {
	t.strictEqual( isEvenf( PINF ), true, 'returns expected value' );
	t.end();
});

tape( 'WARNING: the function returns `true` if provided `-infinity`', function test( t ) {
	t.strictEqual( isEvenf( NINF ), true, 'returns expected value' );
	t.end();
});

tape( 'the function returns `false` if provided `NaN`', function test( t ) {
	t.strictEqual( isEvenf( NaN ), false, 'returns expected value' );
	t.strictEqual( isEvenf( 0.0 / 0.0 ), false, 'returns expected value' );
	t.end();
});
